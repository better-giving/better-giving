...
// imports.
//
// a page of its own under Gifts, reached from the gifts list. it is under Gifts and not the books
// because of what the file holds — what was given, the processor's cut and the money received, per
// gift — while the books screen is where a correction is posted against an entry.
//
...
// about the range, and is only knowable once the file has been asked for: a range nothing was given
// in, one longer than a file holds, one holding two currencies.
// ./_app.admin.donations.export_.journal.ts finds those, and sends the operator back here with the
// reason — `problemWords` below is what words it, and `refused` in the component is what lands them
// on the press it is about, because that return looks exactly like the page they pressed on.
//
...
/** the screen's name, as the document title and the last crumb. no heading draws it. */
 ...
 * range or a range holding one currency either way. the figures stay in the route's text answer for
 * whoever asked it for the file directly.
 *
...
function problemWords(problem: JournalProblem, label: string): string {
	switch (problem) {
		case 'nothing_given':
			return 'Nothing was given between those two days.';
		case 'too_many_rows':
			return `That range is longer than one ${label} file holds. Ask for a shorter range.`;
		case 'mixed_currency':
			return 'That range holds gifts in more than one currency, and one file holds one currency. Ask for a range holding one.';
	}
}
...
export default function ExportGifts({ loaderData }: Route.ComponentProps) {
	const { asked, problem, targets } = loaderData;
	...
	/**
	 * the press a refusal came back about, or nothing where this load carries none.
	 *
	 * a press here downloads, so a refused one answers with a document that looks exactly like the
	 * one it was made on: no file arrives and the page redraws. the reason stands under the row, and
	 * a reader who cannot see it is told nothing at all until they happen to reach that press again
	 * — so the load puts them on it, and `aria-describedby` below is what reads the reason out with
	 * its name.
	 *
	 * on mount and once, which is the whole of it: a refusal only ever arrives on a fresh document,
	 * because the press is a document navigation and there is no in-place state for this to answer.
	 */
	...
	/**
	 * the refusal this load carries: the press it is about, and the words that stand under the row.
	 *
	...
	return (
		...
		//
		// no header, and no prose: the trail above this screen ends in its own name, and a named
		// range over a named row of packages is the whole of what it is. the one line of words below
		// is that row's own name — a sentence about the screen would be it described to somebody
		...
		<Column>
			{/* a `get` at the file's own address, and `reloadDocument` because the answer is a file
			    ...
			    refusing a range before it is asked for. its submit handler is the form's own either
			    way, so a refusal stops the browser's navigation exactly as it stopped the router's. */}
			<Form method="get" action={JOURNAL} reloadDocument discover="none" {...getFormProps(form)}>
				<Stack>
					...
					<Stack tight>
						...
						{/* one sentence and never one per press: the range was shaped for one package, and the
						    ...
						    around it, and none is wanted: it arrives with a document load rather than inside one,
						    so a region would be announced to nobody — the press taking focus is what reads it
						    out. */}
						{refusal === null ? null : (
