...
import { journalFile, type JournalTarget } from '$lib/server/ledger/journal-file';
...
// placement, and the address is under Gifts because what the file holds is what was given —
// the income, the processor's cut and the money received, per gift.
//
...
function sendBack(
	...
	return redirect(`${href('/admin/donations/export')}?${query}`);
}
...
export async function loader({ context, request }: Route.LoaderArgs) {
	...
	// a range nothing was given in is refused rather than shaped. `$lib/server/ledger/journal-file.ts`
	...
	if (groups.length === 0) {
		return onScreen
			? sendBack('nothing_given', asked)
			: refused(422, `nothing was given between ${RANGE_ENDS}.`);
	}
	...
	// about the request.
	if (!file.ok) {
		if (onScreen) return sendBack(file.refusal.reason, asked);
		return refused(
			422,
