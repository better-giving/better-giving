...
function screen(over: Partial<LoaderData> = {}): { root: HTMLElement; posted: FormData[] } {
	...
	return {
		// the route carries no `loader`, so the landing renders in one synchronous pass and every
		// case below reads the screen as soon as it is mounted.
		root: mount(createElement(Stub, { initialEntries: [SCREEN] })),
		posted
	};
}
...
/**
 * the correction's own form, found by the hidden box it carries.
 *
 * the box rather than the element, because what makes the form the right one is the id it posts
 * under: `press` below reads its actions row off this, and a `<form>` found by position would be
 * whichever one the screen drew first.
 */
