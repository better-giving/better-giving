...
// with and never queries — nothing in this app reads the `account` table to name or pick an
// account: every write names one from the map below, and the one screen that offers an account
// to pick is offered the map too, through `pickableAccounts`.
//
...
type SeededAccount<TId extends string = string> = {
	...
	/** verbatim from the seed; the account picker shows it beside the code. */
...
export const POSTING_ACCOUNTS = {
	 ...
	 */
	undepositedFunds: {
		id: '019fb0d2-7d57-7c5e-a7df-baed1f27b405',
		code: '1020',
	...
	salesTaxPayable: {
		id: '019fb0b4-ec6e-7ff7-960c-a442a54222ff',
		code: '2200',
...
export const ROLLUPS = {
	...
	donations: {
		id: '019fb0b4-ec6e-7ff7-960c-a443c29210fa',
		code: '4100',
		name: 'Donations',
		type: 'revenue'
	}
...
/** an account as the correcting entry's picker offers it. */
export type PickableAccount = {
	readonly id: PostableAccountId;
	readonly code: string;
	readonly name: string;
};

/**
 * every account a correction may move money out of or into, ordered by code — which is the
 * chart's own order, assets first.
 *
 * the postable set is exactly `POSTING_ACCOUNTS`: a program is its own `program` row and never an
 * account, and an account a migration adds lands in the map in the same commit.
 */
export function pickableAccounts(): PickableAccount[] {
	return (Object.keys(POSTING_ACCOUNTS) as PostingAccountKey[])
		.map((key) => {
			const { code, name } = POSTING_ACCOUNTS[key];
			return { id: postableId(key), code, name };
		})
		.sort((a, b) => a.code.localeCompare(b.code));
}

/**
 * the branded id of a submitted account pick, or `null` for anything that is not one of
 * `POSTING_ACCOUNTS` — a rollup's id, an id no seed carries, a key name, garbage.
 *
 * built on `postableId` rather than casting, so ./postable.spec.ts's two mints stay two: the id is
 * only ever handed back as the one the matching key already brands.
 */
export function postableIdFromSubmitted(id: string): PostableAccountId | null {
	const key = (Object.keys(POSTING_ACCOUNTS) as PostingAccountKey[]).find(
		(k) => POSTING_ACCOUNTS[k].id === id
	);
	return key === undefined ? null : postableId(key);
}
