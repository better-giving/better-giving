...
//
// all twenty-four values ./env.ts and ../auth/env.ts read are plain Worker vars — which twenty-four,
...
// `pnpm run deploy` uploads a config that mentions none of them and Cloudflare drops all twenty-four,
...
describe('the deploy-time configuration this app is deployed with', () => {
	 ...
	 * every one of the twenty-four is a credential or an org-specific address, and the one that gets
	 * committed will be committed by someone adding a `vars` block for a value they thought was
	 * harmless.
	 */
	it('declares none of the twenty-four anywhere in the file', () => {
		const declared = [
