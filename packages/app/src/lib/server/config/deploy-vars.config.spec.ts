...
//
// all twenty-one values ./env.ts and ../auth/env.ts read are plain Worker vars — which twenty-one,
...
// `pnpm run deploy` uploads a config that mentions none of them and Cloudflare drops all twenty-one,
...
describe('the deploy-time configuration this app is deployed with', () => {
	 ...
	 * every one of the twenty-one is a credential or an org-specific address, and the one that gets
	 * committed will be committed by someone adding a `vars` block for a value they thought was
	 * harmless.
	 */
	it('declares none of the twenty-one anywhere in the file', () => {
		const declared = [
