...
// the gallery's half of the gate that keeps a value out of a screen. every colour and every length
// comes from packages/operator/src/styles/tokens.css, which is what makes a value re-pointable in
// one place and what stops the operator surfaces drifting apart a value at a time.
//
// this file holds only what is the gallery's: which files are swept, and the case that says the
// globs reaching them matched something. the sweep itself, the deny lists behind it and the
// `raw-colour-ok:` / `raw-length-ok:` bargain are packages/operator/src/styles/raw-values.ts's — one
// rule wherever a screen lives, called here with globs of this surface rather than copied.
// packages/console-ui/src/raw-values.spec.ts is the console's own half of the same arrangement, and
...
// reading did not. what this surface owns is its previews and ../app.css, which is where its layer
// order is declared — a colour written there would outrank the layer every sheet's rule is inside.
//
// the length sweep is handed the previews and nothing else, for the reason written beside the
// assertion in packages/operator/src/styles/raw-values.ts: a `1px` border in a sheet is legitimate
// and everywhere, and a rule over the sheets would be mostly allowlist.

describe('no raw value in a gallery screen', () => {
	...
	it('finds the files it is meant to be guarding', () => {
		...
		// the failure the whole file exists to refuse.
