...
//
// **a read that did not land takes the whole page and never a fold.** the twenty-one values come off
...
//
// **the halves overlap rather than queue.** the account's own reads, the twenty-one values and the
...
func Read(ctx context.Context, inputs Inputs) Reading {
	if inputs.Credential.Kind == cf.NoCredential {
		// nothing is asked of the account at all, so what the reading says about why the twenty-one
...
// the twenty-one, which is one read of the worker's own bindings.
...
func assemble(databases DatabaseList, address Address, values Values, report ReportRead) Reading {
	// the twenty-one and the rows are carried on every face and never only on the one that draws
