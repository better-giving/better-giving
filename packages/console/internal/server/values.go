...
// the two presses over the values this deployment is configured with — internal/release's
// DeployVars — one setting them and taking them off, and one freeing a name the deployment is
// holding as a credential.
//
...
//
// The whole enumeration is the largest thing posted here — every name on internal/release's
// DeployVars and the value under it — and anything past that is not this page.
...
func refuseName(w http.ResponseWriter, name string) {
	answer(w, http.StatusBadRequest, map[string]string{
		"error": "this console sets the values a deployment is configured with, which are " +
			"DEPLOY_VARS in packages/operator/src/deploy-split.ts, and " + name +
			" is not one of them",
	})
}
