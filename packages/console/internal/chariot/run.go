...
// the one Chariot setup run this process is holding, and how a page reads it.
//
// ../paypal/run.go's arrangement and for its reasons: the press answers as soon as the chain is under
...
// the secret of whichever subscription was made first, while the second marks it deleted. the
// guarding is internal/run's; what is here is the flat wire shape the page reads.
...
//
// **ctx is the run's own and never the request's**, for ../stripe/run.go's reason. the key is bound
...
func (runs *Runs) Start(ctx context.Context, asked Asked, effects Effects) (Run, bool) {
	...
	started, going := runs.holder.Start(ctx,
		...
		// raised inside a chain holding a key may be spelling it.
		func() Outcome { return Outcome{Kind: ConsoleStopped} })
	return read(&started), going
}

// Read is what this console is doing about Chariot, or nil where it has never been asked.
