...
//
// **what the operator agrees to is the update, and its migrations are part of it.** a release's code
// and its schema move together (../../cmd/better-giving/start.go's carryingOver): an update agreed
// to applies what this release carries to the live database in front of the upload, and one
// declined moves neither. so the question is the same whether or not the release moves the database
// (./carrying), and the screen names no migration. a remote migration still cannot be taken back
// and nothing here rolls one back (CLAUDE.md), so the confirm opens on keeping the current version:
// a hand resting on the keyboard is how a door like this is opened by accident, and every keystroke
// that is not the operator choosing the update leaves the deployment as it stands. what the screen
// does name is the object: an operator whose machine has operated two deployments answers off the
// account and the address on it or not at all.
//
...
//
// **the object is named where nobody is standing, and the door is not opened there.** ./prompt.go's
...
// command that put one there would stand at it waiting on an answer that is not coming. the answer
// is its own, because nobody shut that door: an operator who chose to keep the current version made
// a decision, and a pipe made none.
//
// **a console newer than this one is named above the door and not before it.** the door is drawn on
...
// already taken — so both are handed in rather than read again, and both are drawn above the
// question. a machine that has operated two deployments, or one whose remembered account is not the
// one the operator has in mind, cannot answer "update your deployment?" from a screen that names
// neither.
//
...
const (
	...
	// terminal, or a form that could not be drawn at one. The deployment was named and no question
	// was ever answered, so nothing was refused and nobody refused it.
...
// ConfirmCarry names the deployment a carry would update and takes the operator's answer for it.
//
// `at` is the deployment the answer is about, drawn above the question. `offering` is the release
// this binary would put on it, as ../../cmd/better-giving/main.go's version states it. `ahead` is
// what the database records and this release does not carry, which is ../migrate's. `newer` is the
// line naming a console newer than the one asking, and empty where there is none.
func ConfirmCarry(
	in io.Reader,
	to io.Writer,
	at Deployment,
...
//
// **a release that moves the database is asked about the same way as one that does not.** its
// migrations apply as part of the update, in front of the upload, and a declined update moves
// neither (../../cmd/better-giving/start.go's carryingOver), so the update is the whole of what the
// operator answers for.
//
