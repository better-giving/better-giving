...
func bundleStopped(at deploy.Stage, fix Repair) string {
	...
	case deploy.Uploading, deploy.Pushing, deploy.Scheduling, deploy.Addressing:
		// one sentence for the upload and the address after it: the static files, the app's code, its
		// schedule and somewhere for it to answer are four rows to watch and one state to be left in,
		// since a deployment is what a worker answering somewhere makes and none of them got that far.
