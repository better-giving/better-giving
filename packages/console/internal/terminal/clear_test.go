...
func TestTheMigrationsAreNamedIntoARecordWithoutClearingIt(t *testing.T) {
	...
	var held bytes.Buffer
	if answered := ConfirmMigration(strings.NewReader(""), &held,
		[]string{"0014_thing.sql"}, nil); answered != Declined {
		t.Errorf("ConfirmMigration at a pipe = %q, want %q", answered, Declined)
	}
