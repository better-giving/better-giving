...
`start` runs in the terminal, asking its questions there, and opens the console once the deployment stands. Typed again later it is the same one errand: where a deployment is already standing it carries this release onto it — naming every migration it would apply and waiting for your answer, exactly as `update` does — and then opens the console at it. Everything after that (the thirteen values, Stripe, the widget, the site list, the org identity) is a screen in that console.
