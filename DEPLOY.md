...
**Stripe webhook**: saving both keys on the console's **Stripe** page registers the endpoint at the Worker's address and stores the signing secret Stripe returns exactly once, in the same run. **Without an endpoint, cards are charged and no gift reaches your books**, and that is invisible from the donation form. The same page's Payment notices row says whether Stripe is telling this deployment that gifts are paid. An endpoint switched off, or short of an event after an upgrade, is put right with **Repair** on that row: the endpoint is switched back on and subscribed to every event, and its signing secret is kept, so the press opens no gap of its own. A lost secret or a missing endpoint is repaired the first way: paste the secret key again, **Save**. An endpoint already there is deleted and recreated, so gifts settling in the gap are charged and not recorded; do it when nobody is giving. Repeating gifts ride the same press.
