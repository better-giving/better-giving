...
- **The payout arrives net of NOWPayments' fee.** A donor sends the amount quoted and the account is charged, so a gift a donor did not choose to cover the fee on lands short by it. The rate quoted to a donor is NOWPayments' published 1% (`NOWPAYMENTS_FEE_RULE` in `packages/app/src/lib/server/payments/fees.ts`); what was actually taken is read off each payment, and the chain's own fee for moving the coin to the payout wallet is on top of it and stated nowhere.
