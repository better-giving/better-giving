...
- **Take each processor's pair from one place too.** Stripe's two keys are one pair from one dashboard page, and PayPal's client id and secret are one pair from one app in PayPal's developer dashboard. A mismatched pair is refused by the processor at the first charge, and nothing here checks it for you. A [rehearsal deployment](#a-rehearsal-deployment) points `PAYPAL_API_URL` at PayPal's sandbox address, `https://api-m.sandbox.paypal.com`, and takes its PayPal pair from a sandbox app; unset, it is `https://api-m.paypal.com`.
