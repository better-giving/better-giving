...
**Nothing shows a refused PayPal delivery either.** PayPal's own developer dashboard lists the deliveries it attempted, and that list is where a refused or unanswered one is visible; the **Save** press above is the answer on doubt.
...
`start` runs in the terminal, asking its questions there, and opens the console once the deployment stands. It asks which Cloudflare account this deployment is in on every run, opening on the one this machine remembers. Typed again later it is the same one errand: where a deployment is already standing it compares the release that deployment reports with the one this console carries, and where they differ it offers to carry this release onto it — its migrations applied as part of that update, and only once you agree to it — and then opens the console at it either way, already connected. Connecting signs out any other console connected to that deployment, and `start` says so as it does it; on a deployment already on this release it asks nothing and goes straight to connecting. Everything after that (the seventeen values, Stripe, the widget, the site list, the org identity) is a screen in that console.
