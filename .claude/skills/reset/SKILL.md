...
3. **Delete this deployment's PayPal webhook endpoint.** The same reason as step 2 and a different mechanism: the console's PayPal **Save** registers this listener (`packages/console/internal/paypal/setup.go`) and nothing in the tree ever deletes it, so it outlives the worker. The address is `https://<name>.<subdomain>.workers.dev/api/paypal/webhook`, `<subdomain>` as read in step 2, and there is no CLI for it:

   - **The host** — `<paypal_api>` is `PAYPAL_API_URL` from `packages/app/.dev.vars` where it is set (a sandbox app's keys take `https://api-m.sandbox.paypal.com`), else `https://api-m.paypal.com`. The keys follow that address and nothing else, so a 401 below is keys that do not belong to that host.
