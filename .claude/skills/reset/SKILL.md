...
3. **Delete this deployment's PayPal webhook endpoint.** The same reason as step 2 and a different mechanism: nothing in this app ever created this one — an operator registers it by hand in PayPal's own developer dashboard (DEPLOY.md) — so nothing in this app deletes it either, and it outlives the worker with no record anywhere in the tree that it existed. The address is `https://<name>.<subdomain>.workers.dev/api/paypal/webhook`, `<subdomain>` as read in step 2, and there is no CLI for it:

   - **The host** — `<paypal_api>` is `PAYPAL_API_URL` from `packages/app/.dev.vars` where it is set (a sandbox app's keys take `https://api-m.sandbox.paypal.com`), else `https://api-m.paypal.com`. The keys follow that address and nothing else: the only test-versus-live read in this repo is the Stripe dispute alert's dashboard link, which follows the dispute's own `livemode`, so a 401 below is keys that do not belong to that host.
   - **The token** — `PAYPAL_CLIENT_ID` and `PAYPAL_CLIENT_SECRET` from `packages/app/.dev.vars`, each read into a shell variable on a line of its own so neither reaches the transcript, then exchanged for a bearer at `POST <paypal_api>/v1/oauth2/token` with `grant_type=client_credentials`.
   - **The calls** — `GET <paypal_api>/v1/notifications/webhooks` lists what the account holds, then `DELETE <paypal_api>/v1/notifications/webhooks/<id>` for each entry whose `url` is the address. A delete answers `204` with no body, so there is nothing to read back.

   Match on the address rather than on `PAYPAL_WEBHOOK_ID` in `.dev.vars`: that id names the endpoint the deployment verified deliveries against when the file was last written, and an endpoint re-registered since leaves it stale — the stale id deletes nothing while the live endpoint stays behind. Done when the list no longer holds the address; no match is already-gone. Every other address the list carries is another deployment's and stays.


4. **Delete the app's worker.** From the repo root: `pnpm wrangler delete` — the name comes from `wrangler.jsonc`, every secret and var dies with the worker, and a non-interactive run auto-confirms. Done on `Successfully deleted <name>`, or on `code: 10007` — Cloudflare's "this Worker does not exist on your account", which is already-gone.

5. **Delete the donation page's worker a set-up made before the page moved into the app may have left.** `pnpm wrangler delete --name <name>-donate`. No press puts one up any more — the donation page is the app's own route, `/{form_id}` (`packages/app/src/routes/$formId.tsx`) — but a deployment made when one did still holds it, and left behind it keeps answering with a page pointing at a deployment that no longer exists. Done on step 4's two signals, and **`10007` is the ordinary outcome here** — it arrives as a failed command, pnpm exiting non-zero over its own wrapper error with Cloudflare's reason several lines above, so read the output for the code rather than the exit status. The step can go once no deployment from before that change is left.

6. **Delete the database.** `pnpm wrangler d1 delete <database_name> -y`. This is the one-way door of the run — the records are unrecoverable, which is what a reset is for. Done when wrangler confirms or the name is already absent.

7. **Wipe local console state.** Delete the directory `$BETTER_GIVING_HOME` if set, else the os config home (`~/Library/Application Support/better-giving` on macos). It holds the account claim, the oauth token and the deployment session, so the next `pnpm console` starts at the Cloudflare allow page. Done when the directory is gone.

8. **Rebake the console's config.** `go run ./cmd/bake` from `packages/console`. The pack reads the committed `internal/release/config.json`, so the bake is what carries a migration added since the last one into the bundle (`cmd/bake/main.go` states the arrangement). Done when the bake names the file and the commit it stamped; a changed file is the user's to commit.

9. **Pack the worker bundle.** `pnpm run bundle` from the repo root. A checkout-built binary is version `dev`, so the deploy press looks for a release that was never cut and refuses with `that release carries no worker-dev.tar.gz`; this writes the one it reads instead. It is the slow step — two builds and a dry run — and it is here because a reset is the moment the next deploy is certain to want a bundle built from the tree as it stands. Done when the script names the archive it wrote and its module, asset and migration counts.

10. **Verify and report.** `pnpm wrangler d1 list` prints nothing under its own banner, the three ports are quiet, and the bundle holds every migration in the tree: `tar tzf packages/console/bundle/worker-dev.tar.gz | grep -c 'migrations/.*sql'` equals `ls packages/app/migrations/*.sql | wc -l`. Report each target as deleted or already-gone, then name the next command: `pnpm console`.

## What survives, deliberately

- **The turnstile widget** — the next set-up press adopts it by name (`packages/console/internal/first`).
- **Other deployments' webhook endpoints** — the same processor account at other addresses, each somebody else's set-up, and steps 2 and 3 touch exactly one each. One whose deployment is gone is deleted in that processor's own dashboard.
