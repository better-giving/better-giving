...
5. **Start the release.** Dispatch `release.yml` on `main` with the inputs `tag` and `notes`: the GitHub MCP `actions_run_trigger` (`run_workflow`), the REST call `POST /repos/better-giving/better-giving/actions/workflows/release.yml/dispatches`, or the Actions tab's "Run workflow". Done when the newest `workflow_dispatch` run of `release.yml` is on step 1's commit.
   ...
   **A hand-pushed tag is the other way in**, for someone at their own machine who chooses it. The file holds the tag on its first line, a blank line, then step 4's paragraph. Run `git tag -a <tag> <sha> -F <file>` against step 1's commit, then `git push origin <tag>`. It skips the run's gates, so steps 1–3 are the only gate that path has.

6. **The run, watched through.** It is the heaviest thing this repository runs — a pnpm install, the app's vite build, wrangler, and four cross-builds, several minutes in all — so watch it with one background loop over that run's status (`actions/runs/<id>`) that reads the result once at the end. Done when it concludes `success`; a failure goes to the section below.

7. **All seven assets, read off the release** (`releases/tags/<tag>`): four `better-giving_{darwin,linux}_{amd64,arm64}.tar.gz`, `checksums.txt`, `worker-<version>.tar.gz`, `install.sh`, and `prerelease` false. **Every release carries `install.sh`**, because `releases/latest/download/install.sh` resolves to the newest release's copy — one without it takes the install line down for everyone, not only for itself. Then `git fetch origin --tags`, deleting first any local tag of that name the remote's doesn't match. Done when all seven are named, the flag is off, and the local tag is the remote's.
